...
from bank_tools.dispatch import SPECS, Spec, _s, dispatch
...
def build_specs(intel: MerchantIntel, reports: workflow.ReportStore, messenger: Messenger) -> dict[str, Spec]:
    ...
    return {
        ...
        "escalate_case": Spec(lambda store, case_id, reason, idempotency_key: workflow.escalate_case(store, case_id=case_id, reason=reason),
                              (_s("caseId", "case_id"), _s("reason", "reason"), _IDEM)),
